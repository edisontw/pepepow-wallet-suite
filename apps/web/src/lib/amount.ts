...
export type FormatAtomicOptions = {
  group?: boolean;
...
export function parsePepewToAtomic(input: string, decimals = PEPEW_DECIMALS): bigint {
  if (!Number.isInteger(decimals) || decimals < 0) {
    throw new Error(`Invalid decimals: ${decimals}`);
  }
  const raw = String(input ?? "");
  const normalized = raw.trim().replace(/,/g, "").replace(/\s+/g, "");
  if (!normalized) throw new Error("Invalid decimal amount: empty");
  if (normalized.includes("-")) throw new Error(`Invalid decimal amount: negative value ${input}`);
...
export function formatAtomicToPepew(
  atomic: bigint,
  decimals = PEPEW_DECIMALS,
  opts: FormatAtomicOptions = {},
): string {
  if (!Number.isInteger(decimals) || decimals < 0) {
    throw new Error(`Invalid decimals: ${decimals}`);
  }
...
export function addAtomic(a: bigint, b: bigint): bigint {
  return a + b;
}

export function cmpAtomic(a: bigint, b: bigint): -1 | 0 | 1 {
  if (a < b) return -1;
  if (a > b) return 1;
  return 0;
}
