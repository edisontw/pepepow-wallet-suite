...
import { cancelNestExOrder } from "../exchanges/nestex.js";
...
export async function cancelOutstandingOrders(configId: number): Promise<{ cancelled: number; failed: number; total: number; alreadyClosed: number; remaining: number }> {
    ...
    const toCancelIds = new Set<string>(localOpenOrders.map(o => o.order_id));
    ...
    for (const orderId of toCancelIds) {
        ...
        if (exchange === "nonkyc") {
            ...
            const res = await cancelDexTradeOrder(apiKey, apiSecret, orderId);
            if (res.ok) {
                status = "CANCELLED";
