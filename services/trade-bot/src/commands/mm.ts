...
export async function handleMmStart(ctx: Context): Promise<void> {
    ...
    try {
        ...
        await safeSend(ctx, { step: "mm_start.error", text: "❌ Failed to start MM. Please try again." });
    }
}
...
export async function handleMmCallback(ctx: Context): Promise<boolean> {
    ...
    if (action === "pair" && value) {
        ...
        await safeSend(ctx, {
            step: "mm.spread",
            text: `Enter spread percentage for ${pairDisplay} (e.g. 1 for 1%):`,
        });
    ...
    if (action === "ops" && value) {
        ...
        try {
            ...
            const lines = [
                "✅ MM config created and started (REAL):",
            ...
            await safeSend(ctx, { step: "mm.create.error", text: "❌ Failed to create MM config. Please try again." });
...
export async function handleMmTextInput(ctx: Context): Promise<boolean> {
    ...
    if (state.step === "spread") {
        ...
        if (!Number.isFinite(value) || value <= 0) {
            await safeSend(ctx, { step: "mm.spread.invalid", text: "❌ Spread must be a positive number." });
            return true;
        }
        // User input is percentage (e.g., 1 means 1%), store as ratio (0.01)
        const spreadPct = value / 100;
        if (value > 20) {
            await safeSend(ctx, { step: "mm.spread.warning", text: `⚠️ Spread ${value}% is unusually high. Proceeding anyway (Experimental).` });
        }
        state.spreadPct = spreadPct;
        state.step = "order";
        state.updatedAt = Date.now();
        pendingMm.set(tgUserId, state);
